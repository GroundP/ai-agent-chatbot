...
from tools import web_search_tool, naver_search_tool
...
@CrewBase
class ChatBotCrew:
    # agents_config = "config/agents.yaml"  # yaml 파일을 사용하여 에이전트 설정(현재는 코드베이스로 진행행)
    ...
    def communication_agent(self) -> Agent:
        return Agent(
            ...
            """,
            llm="openai/o4-mini",
            tools=[web_search_tool, naver_search_tool],
            verbose=True,
        )
